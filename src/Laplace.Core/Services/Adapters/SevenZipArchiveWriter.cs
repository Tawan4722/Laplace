...
public sealed class SevenZipArchiveWriter
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        {
            await CreateExternalAsync(executable, inputPaths, scanned, outputArchivePath, options, progress, cancellationToken).ConfigureAwait(false);
            return;
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (commonParent is not null)
        {
            ...
            await RunSevenZipAsync(executable, commonParent, Path.GetFullPath(outputArchivePath), options, relativePaths, cancellationToken).ConfigureAwait(false);
            progress?.Report(new ArchiveOperationProgress
            {
        ...
        try
        {
            ...
            var entries = Directory.EnumerateFileSystemEntries(stagingRoot)
                ...
                .Select(x => x!);
            await RunSevenZipAsync(executable, stagingRoot, Path.GetFullPath(outputArchivePath), options, entries, cancellationToken).ConfigureAwait(false);
            progress?.Report(new ArchiveOperationProgress
            {
                CurrentItem = Path.GetFileName(outputArchivePath),
